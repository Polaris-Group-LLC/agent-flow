...
import ComposioNode, {
  ComposioNodeData,
} from "@/components/builder-nodes/ComposioNode";
...
export default function BuilderPage() {
  ...
  const sidebarNodeTypes = [
    ...
    },
    {
      ...
      defaultData: { label: "LLM Call", systemPrompt: "", apiKey: "" },
    },
    {
      type: "composio",
      label: "Composio Tool",
      icon: <Puzzle size={18} />,
      defaultData: { label: "Composio Action", apiKey: "", toolAction: "" },
    },
    {
      type: "customOutput",
      label: "Output Node",
  ...
  const allSidebarItems = [
    ...
    })),
    {
      key: "augmented-llm",
      label: "Augmented LLM",
      icon: <span className="text-lg font-bold text-foreground">A</span>,
    ...
    },
    {
      ...
      icon: <span className="text-lg font-bold text-foreground">C</span>,
    ...
    },
    {
      key: "routing",
      label: "Routing",
      icon: <span className="text-lg font-bold text-foreground">R</span>,
    ...
    },
    {
      ...
      icon: <span className="text-lg font-bold text-foreground">P</span>,
    ...
    },
    {
      ...
      icon: <span className="text-lg font-bold text-foreground">E</span>,
  ...
  const handleCreateWorkflowFromCommand = useCallback(
    ...
    () => ({
      ...
      llm: (props) => (
        <LLMNode
          {...props}
          data={{ ...props.data, onNodeDataChange: onNodeDataChange as any }}
          onCopyApiKeyToAllLLMs={(apiKey) =>
            setNodes((nds) =>
              nds.map((n) =>
                n.type === "llm" ? { ...n, data: { ...n.data, apiKey } } : n
              )
            )
          }
        />
      ...
      composio: (props) => (
        <ComposioNode
          {...props}
          data={{ ...props.data, onNodeDataChange: onNodeDataChange as any }}
          onOpenToolsWindow={(apiKey?: string) => {
            setToolsWindowOpen(true);
            setCurrentComposioApiKey(apiKey || "");
          }}
          onCopyApiKeyToAllComposioNodes={(apiKey) =>
            setNodes((nds) =>
              nds.map((n) =>
                n.type === "composio"
                  ? { ...n, data: { ...n.data, composioApiKey: apiKey } }
                  : n
              )
            )
          }
      ...
      agent: (props) => (
        <AgentNode
          ...
          onCopyApiKeyToAllAgents={(apiKey) =>
            setNodes((nds) =>
              nds.map((n) =>
                n.type === "agent"
                  ? { ...n, data: { ...n.data, llmApiKey: apiKey } }
                  : n
              )
            )
          }
  ...
  function getPatternNodesAndEdges(
    patternType: string,
    position: XYPosition,
    getUniqueNodeId: (type: string) => string,
    onNodeDataChange: any
  ) {
        ...
        nodes: [
          {
            ...
            type: "customInput",
            position: { x, y },
            data: { label: "Input", query: "", onNodeDataChange, width: 160 },
          },
          {
            id: llmId,
            type: "llm",
            position: { x: x + hSpacing, y },
            data: {
              label: "LLM",
              systemPrompt: "",
              apiKey: "",
          ...
          },
          {
            id: outputId,
            type: "customOutput",
            position: { x: x + hSpacing * 2, y },
            data: { label: "Output", width: 160 },
          },
        ],
        edges: [
      ...
      return {
        nodes: [
          ...
          },
          {
            ...
            type: "agent",
            position: { x: x + hSpacing, y },
            data: { label: "Agent 1", onNodeDataChange, width: 180 },
          },
          {
            id: agent2Id,
            type: "agent",
            position: { x: x + hSpacing * 2, y },
            data: { label: "Agent 2", onNodeDataChange, width: 180 },
          },
          {
            id: outputId,
            type: "customOutput",
            position: { x: x + hSpacing * 3, y },
            data: { label: "Output", width: 160 },
          },
        ],
        edges: [
        ...
        nodes: [
          {
            id: inputId,
            type: "customInput",
            position: { x, y: y + vSpacing },
            data: { label: "Input", query: "", onNodeDataChange, width: 160 },
          },
          {
            id: routerId,
            type: "agent",
            position: { x: x + hSpacing, y: y + vSpacing },
            data: { label: "Router", onNodeDataChange, width: 180 },
          },
          {
            id: agent1Id,
            type: "agent",
            position: { x: x + hSpacing * 2, y: y },
            data: { label: "Agent 1", onNodeDataChange, width: 180 },
          },
          {
            id: agent2Id,
            type: "agent",
            position: { x: x + hSpacing * 2, y: y + vSpacing * 2 },
            data: { label: "Agent 2", onNodeDataChange, width: 180 },
          },
          {
            id: outputId,
            type: "customOutput",
            position: { x: x + hSpacing * 3, y: y + vSpacing },
            data: { label: "Output", width: 160 },
          },
        ],
        edges: [
      ...
      return {
        nodes: [
          ...
          },
          {
            id: agent1Id,
            type: "agent",
            position: { x: x + hSpacing, y: y },
            data: { label: "Agent 1", onNodeDataChange, width: 180 },
          },
          {
            id: agent2Id,
            type: "agent",
            position: { x: x + hSpacing, y: y + vSpacing * 2 },
            data: { label: "Agent 2", onNodeDataChange, width: 180 },
          },
          {
            id: aggregatorId,
            type: "llm",
            position: { x: x + hSpacing * 2, y: y + vSpacing },
            data: { label: "Aggregator", onNodeDataChange, width: 160 },
          },
          {
            id: outputId,
            type: "customOutput",
      ...
      return {
        nodes: [
          ...
          },
          {
            ...
            type: "llm",
            position: { x: x + hSpacing, y },
            data: { label: "Generator", onNodeDataChange, width: 160 },
          },
          {
            id: solutionId,
            type: "customOutput",
            position: { x: x + hSpacing * 2, y },
            data: { label: "Solution", width: 160 },
          },
          {
            id: evaluatorId,
            type: "llm",
            position: { x: x + hSpacing, y: y + vSpacing },
            data: { label: "Evaluator", onNodeDataChange, width: 160 },
          },
        ],
        edges: [
  ...
  const expandPatternMetaNode = (
    metaNodeId: string,
    patternType: string,
    position: XYPosition
  ) => {
    const { nodes: patternNodes, edges: patternEdges } =
      getPatternNodesAndEdges(
        patternType,
        position,
        getUniqueNodeId,
        onNodeDataChange
    ...
    const centroid = patternNodes.reduce(
    ...
    setNodes((nds) =>
      nds.filter((n) => n.id !== metaNodeId).concat(nodesWithCollapse)
    );
    ...
    reader.onload = (e) => {
      try {
        ...
        if (
          ...
          throw new Error(
            "Invalid workflow file format. Missing 'nodes' or 'edges' array."
          );
        }
  ...
  useEffect(() => {
    const hasInput = nodes.some(
      (n) => n.type === "customInput" || n.type === "input"
    );
    const hasOutput = nodes.some(
      (n) => n.type === "customOutput" || n.type === "output"
    );
  ...
  const handleRunAgentFromBuilder = async () => {
    ...
    ) as Node<InputNodeData> | undefined;
        ...
        nds.map((node) =>
          node.type === "customOutput"
            ? {
                ...
                data: {
                  ...node.data,
                  agentOutput: responseData.response || "",
                },
              }
            : node
        )
  ...
  const handleAddActionsToAgent = (actions: any[]) => {
    ...
    const node = selectedNodes.find(
      (n: Node) => n.type === "agent" || n.type === "composio"
    );
    if (!node) return;
  ...
  const joyrideSteps = [
    {
      ...
      content:
        "This is the Node Library. Drag nodes onto the canvas to build your flow.",
  ...
  return (
    ...
    >
      ...
      {/* Top Bar */}
      <header
        className="h-16 flex items-center justify-between px-6 shrink-0 bg-background/70 border-b border-border"
        style={{
          backdropFilter: "blur(10px)",
        }}
      >
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-1.5 rounded-md transition-all duration-200 hover:bg-foreground/20 hover:scale-105 bg-foreground/10 text-foreground"
            style={{
          ...
          {editingName ? (
            <form onSubmit={handleNameSubmit} className="ml-2">
              <input
                ...
                className="text-xl font-semibold bg-transparent border-b border-primary text-foreground focus:outline-none px-1 w-48 focus:text-foreground hover:text-foreground"
              />
            </form>
          ) : (
            <span
              className="text-xl font-semibold text-foreground ml-2 cursor-pointer hover:text-foreground"
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 text-sm rounded-md transition-all duration-200 flex items-center gap-1.5 hover:bg-foreground/20 hover:scale-105 bg-foreground/10 text-foreground"
            style={{
              backdropFilter: "blur(5px)",
            }}
          ...
          <button
            ...
            className="px-3 py-1.5 text-sm rounded-md transition-all duration-200 flex items-center gap-1.5 hover:bg-foreground/20 hover:scale-105 relative bg-foreground/10 text-foreground"
            style={{
              backdropFilter: "blur(5px)",
            }}
          ...
          />
          <button
            className="px-3 py-1.5 text-sm rounded-md transition-all duration-200 hover:bg-foreground/20 hover:scale-105 bg-foreground/10 text-foreground"
            style={{
              backdropFilter: "blur(5px)",
            }}
          >
      ...
      <div
        className="flex items-center px-6 py-2 bg-background/70 border-b border-border/50"
        style={{
          backdropFilter: "blur(10px)",
        }}
      >
      ...
      <div className="flex flex-grow min-h-0">
        ...
        <aside
          ...
          className={`flex flex-col shrink-0 overflow-y-auto transition-all duration-300 ease-in-out bg-background/70 border-r border-border/50
                      ${
                        isLeftSidebarOpen
                          ? "w-72 p-4"
        ...
        >
          {isLeftSidebarOpen && (
            <>
              <div className="sticky top-0 z-20 pb-2 mb-3 flex flex-col gap-2 bg-background/70 border-b border-border/50">
                ...
                <Input
                  ...
                  className="mt-1 bg-background/40 border border-border text-foreground placeholder:text-foreground/40"
                />
              </div>
              <div
              ...
              >
                {filteredSidebarItems.map((item) => (
                  <div
                    ...
                    data-tutorial={
                      item.dragType === "node" &&
                      ((item.dragData as any).nodeType === "customInput"
                        ? "input-node"
                        : (item.dragData as any).nodeType === "llm"
                        ? "llm-node"
                        : undefined)
                    }
                    onDragStart={(event) => {
                    ...
                    draggable
                    className="group relative p-4 rounded-xl cursor-grab active:scale-[0.97] transition-all duration-200 bg-foreground/10 border border-border"
                    style={{
                      backdropFilter: "blur(5px)",
                    }}
                  >
                    <div className="flex flex-col items-center text-center gap-2">
                      <div
                        className="w-12 h-12 rounded-lg flex items-center justify-center mb-1 bg-foreground/15"
                        style={{
                          backdropFilter: "blur(5px)",
                        }}
                      >
                    ...
                    <div
                      className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity bg-foreground/5 border border-foreground/30"
                      style={{
                        backdropFilter: "blur(5px)",
                      }}
                    />
        ...
        >
          ...
          {toolsWindowOpen && (
            <ToolsWindow
              ...
              composioApiKey={
                currentComposioApiKey || getSelectedComposioApiKey()
              }
            />
          )}

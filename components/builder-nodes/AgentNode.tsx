...
export interface AgentNodeData {
  ...
  onNodeDataChange?: (
    id: string,
    data: Partial<Omit<AgentNodeData, "onNodeDataChange">>
  ) => void;
  width?: number;
}
...
const AgentNode: React.FC<AgentNodeProps & { _forceRerender?: number }> = ({
  id,
  data,
  isConnectable,
  onOpenToolsWindow,
  ...
  const selectedActionsList = (data.allowedTools || "")
    .split(",")
    .map((t) => t.trim())
    .filter(Boolean);
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div
            style={{
              borderBottom: "1px dashed rgba(180, 245, 245, 0.1)",
          ...
          >
            ...
            <div>
              <label htmlFor={`modelProvider-${id}`} style={labelStyle}>
                Model Provider
              </label>
              <select
                id={`modelProvider-${id}`}
            ...
            <div>
              <label htmlFor={`modelName-${id}`} style={labelStyle}>
                Model Name
              </label>
              <select
                id={`modelName-${id}`}
                ...
                value={
                  data.modelName ||
                  modelOptions[data.modelProvider || "openai"][0]
                }
                onChange={handleNodeConfigChange}
                style={inputStyle}
                className="focus:ring-1 focus:ring-[#cbfcfc] cursor-pointer"
              ...
              >
                {(modelOptions[data.modelProvider || "openai"] || []).map(
                  (model) => (
            ...
            <div>
              <label htmlFor={`llmApiKey-${id}`} style={labelStyle}>
                LLM API Key
              </label>
              <div
                style={{
            ...
            <div>
              <label htmlFor={`systemPrompt-${id}`} style={labelStyle}>
                System Prompt
              </label>
              <div
                style={{
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {selectedActionsList.map((actionKey) => (
                <div
                  key={actionKey}
                  className="bg-[#cbfcfc22] text-[#cbfcfc] rounded-full px-3 py-1 text-xs flex items-center gap-1.5"
                >
                  <button
                    type="button"
